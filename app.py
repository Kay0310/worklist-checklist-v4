...
num_units = st.number_input("단위작업공정 개수", min_value=1, step=1)
group_name = st.text_input("반 이름 입력")

for i in range(num_units):
    with st.expander(f"단위작업공정 {i+1} 입력"):
        회사명 = st.text_input(f"[{i+1}] 회사명")
        소속 = st.text_input(f"[{i+1}] 소속")
        반 = st.text_input(f"[{i+1}] 반")
        작업명 = st.text_input(f"[{i+1}] 단위작업명")
        작업자수 = st.number_input(f"[{i+1}] 작업자 수", min_value=1, step=1)
        작업자이름 = st.text_input(f"[{i+1}] 작업자 이름")
